...
def bg_change(image, list_of_textures):
    '''
    Функция аугментирования изображения путем замены белого фона на текстуру
    '''
...
def augmentation_watermarks(img, watermark_path='./augmentation_results/watermarks'):
    '''
    Функция для аугментирования изображений путем добавления вотермарки
    '''
    watermarks_list = []
...
def augmentation_axis(img):
    '''
    Функция для аугментирования изображений путем добавления осей
    '''
    x = 0
...
def blur(image, image2, list_of_textures):
    '''
    Функция аугментирования изображения путем размытия
    '''
...
def pixelize(image, image2, list_of_textures):
    '''
    Функция аугментирования изображения путем пикселизации
    '''
    if random.choice([1, 0, 0, 0, 0]) == 1:
        image = bg_change(image2, list_of_textures)
    if randint(1, 100) <= 10:
...
def change_bc(image, image2, list_of_textures):
    '''
    Функция аугментирования изображения путем изменения контрастности (эффект скана)
    '''
    if random.choice([1,0,0,0,0]) == 1:
        image = bg_change(image2, list_of_textures)
    if randint(1, 100) <= 10:
...
def noize(image, image2, list_of_textures):
    '''
    Функция аугментирования изображения путем добавления шума
    '''
    if random.choice([1, 0, 0, 0, 0]) == 1:
        image = bg_change(image2, list_of_textures)
    if randint(1, 100) <= 10:
...
def invert_image(image):
    '''
    Функция аугментирования изображения путем инвертирования
    '''
...
def augment_image(
    ...
    '''
    Функция для аугментирования одного изображения рандомными способами
    '''
...
def augment_image_rotation(
        ...
        out_path_mask='./augmentation_results/augmented_masks'):
    '''
    Функция для аугментирования изображения путем поворота
    '''
    image = cv2.imread(image_path)
    image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    mask = cv2.imread(mask_path)
...
def texture_to_img(
    ...
    '''
    Функция для нанесения на изображение текстур по маске в соответствии с заданным стилем: empty, grey, line_color, lining
    '''
    image = cv2.imread(image_path)
    mask = cv2.imread(mask_path)
