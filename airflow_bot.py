...
#детали подключения, чат и токен бота удалены из финального кода
...
def vasilenko_bot_dag():
    ...
    def test_report(chat=None):
        ...
        msg = (f"Дата: {data['date'].astype('string')[0]}"
               f"\nМетрики:"
        ...
        msg2 = f"Графики за период {data['date'].astype('string')[0]} - {data['date'].astype('string')[data.index[-1]]}"
